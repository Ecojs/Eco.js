import { AdminReturnModel, IAdminReturnModel } from '../../structures/Eco/WebServer/Web/Models/AdminReturnModel';
import { ExportGameModel, IExportGameModel } from '../../structures/Eco/WebServer/Web/Models/ExportGameModel';
import ECO from '../ECO';
...
export class AdminController extends ControllerBase {
  ...
  public async setServername(name: string) {
    return this.POST<string, string, void>(`/api/v1/admin/set/servername?name=${name}`, undefined)
  }
  ...
  public async setAccess(visibility: 'private', password: string): Promise<string>
  public async setAccess(visibility: 'public' | 'private' | 'hidden', password?: string): Promise<string> {
    let query = `?value=${visibility}`
    if (visibility == 'private') query += `&password=${password}`
    return this.POST<string, string, void>(`/api/v1/admin/set/access${query}`, undefined);
  }
  // public async export() {
